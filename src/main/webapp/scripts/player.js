...
AudioPlayer.prototype = {
    ...
    initialize: function() {
        ...
        ['ended', 'error'].each(function(e) {
            that.player.addEventListener(e, function() {
                audioEndedListener(that, e)
            }, false);
        });
    },
    ...
    /*
     * Restarts playing after x seconds if timer not explicitly cleared, should
     * hopefully catch most load-errors..
     */
...
/*
 * Event-listener that records the end-of-playing and starts playing enqueued
 * sounds.
 */
function audioEndedListener(that, reason) {
    ...
    // play next from queue, but make sure the current event has been fully
    // processed.
    setTimeout(function() {
        that.playNext();
    }, 0);
